...
class SequentialRunner(Runner):
    ...
    def run(
        ...
        try:
            for step in workflow.steps:
                state = self.run_step(workflow, state, step.name, user)
        finally:
    ...
    def run_step(
        ...
        input_atoms_spec = self._resolve_step_inputs(step, state)
    ...
    def _resolve_step_inputs(self, step: Step, state: WorkflowState) -> dict[str, str]:
